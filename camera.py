...
#            face_frame = image[y+y+h, x:x+w]
            face_frame = image[descaledY:descaledY +
                               descaledH, descaledX:descaledX+descaledW]
            ...
            no_of_mouths = len(
                mouth_cascade.detectMultiScale(gray_frame, 1.9, 2))
            ...
            if wearing_mask == True:
                face_label = "Wearing mask"
                image = cv2.rectangle(image, (x, y), (x+w, y+h), (0, 255, 0), 3)
            else:
                face_label = "No Mask"
                image = cv2.rectangle(image, (x, y), (x+w, y+h), (0, 0, 255), 3)

            image = cv2.putText(image, face_label, (x, y - 10),
                                cv2.FONT_HERSHEY_COMPLEX_SMALL, 1.25, (255, 0, 0))
            break
